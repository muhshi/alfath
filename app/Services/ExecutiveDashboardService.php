...
class ExecutiveDashboardService
{
    ...
    /**
     * Retrieve complete executive dashboard metrics for Sensus Ekonomi 2026.
     */
    public function getDashboardData(): array
    {
        ...
        try {
            ...
            if ($schema->hasTable('monitoring_se2026')) {
                ...
                // Wilkerstat BKU per kecamatan from sipw
                $bkuPerKec = [];
                if ($schema->hasTable('sipw')) {
                    $bkuPerKec = $db->table('sipw')
                        ->select(DB::raw("LEFT(id_subsls, 7) as kodekec, SUM(CAST(bku AS SIGNED)) as total_bku"))
                        ->groupBy(DB::raw("LEFT(id_subsls, 7)"))
                        ->pluck('total_bku', 'kodekec')
                        ->toArray();
                }

                // Usaha Keluarga Ditemukan per kecamatan
                $ukTable = $schema->hasTable('se2026_usaha_keluarga') ? 'se2026_usaha_keluarga' : ($schema->hasTable('usaha_keluarga') ? 'usaha_keluarga' : null);
                $ukPerKec = [];
                if ($ukTable) {
                    $latestUkDate = $db->table($ukTable)->whereRaw('LENGTH(kode) >= 7')->max('tanggal_data');
                    $ukPerKec = $db->table($ukTable)
                        ->whereRaw('LENGTH(kode) >= 7')
                        ->when($latestUkDate, function ($q, $d) {
                            return $q->where('tanggal_data', $d);
                        })
                        ->select(DB::raw("
                            LEFT(kode, 7) as kodekec,
                            SUM(CAST(IFNULL(jumlah_usaha_keluarga_menurut_status_keberadaan_usaha___ditemuka, 0) AS SIGNED) + CAST(IFNULL(jumlah_usaha_keluarga_menurut_status_keberadaan_usaha___baru, 0) AS SIGNED)) as total_uk
                        "))
                        ->groupBy(DB::raw("LEFT(kode, 7)"))
                        ->pluck('total_uk', 'kodekec')
                        ->toArray();
                }

                // Usaha Perusahaan Ditemukan per kecamatan
                $upTable = $schema->hasTable('se2026_usaha_perusahaan') ? 'se2026_usaha_perusahaan' : ($schema->hasTable('usaha_perusahaan') ? 'usaha_perusahaan' : null);
                $upPerKec = [];
                if ($upTable) {
                    $latestUpDate = $db->table($upTable)->whereRaw('LENGTH(kode) >= 7')->max('tanggal_data');
                    $upPerKec = $db->table($upTable)
                        ->whereRaw('LENGTH(kode) >= 7')
                        ->when($latestUpDate, function ($q, $d) {
                            return $q->where('tanggal_data', $d);
                        })
                        ->select(DB::raw("
                            LEFT(kode, 7) as kodekec,
                            SUM(CAST(IFNULL(status___ditemukan, 0) AS SIGNED) + CAST(IFNULL(status___baru, 0) AS SIGNED)) as total_up
                        "))
                        ->groupBy(DB::raw("LEFT(kode, 7)"))
                        ->pluck('total_up', 'kodekec')
                        ->toArray();
                }
                ...
                foreach ($this->kecNameMap as $code => $name) {
                    ...
                    $bku = isset($bkuPerKec[$code]) ? (int) $bkuPerKec[$code] : 0;
                    $uk = isset($ukPerKec[$code]) ? (int) $ukPerKec[$code] : 0;
                    $up = isset($upPerKec[$code]) ? (int) $upPerKec[$code] : 0;
            ...
            $ukTable = $schema->hasTable('se2026_usaha_keluarga') ? 'se2026_usaha_keluarga' : ($schema->hasTable('usaha_keluarga') ? 'usaha_keluarga' : null);
            if ($ukTable) {
                $latestUkDate = $db->table($ukTable)
                    ->whereRaw('LENGTH(kode) = 16')
                    ->max('tanggal_data');

                $ukRaw = $db->table($ukTable)
                    ->whereRaw('LENGTH(kode) = 16')
            ...
            $upTable = $schema->hasTable('se2026_usaha_perusahaan') ? 'se2026_usaha_perusahaan' : ($schema->hasTable('usaha_perusahaan') ? 'usaha_perusahaan' : null);
            if ($upTable) {
                $latestUpDate = $db->table($upTable)
                    ->whereRaw('LENGTH(kode) = 16')
                    ->max('tanggal_data');

                $upRaw = $db->table($upTable)
                    ->whereRaw('LENGTH(kode) = 16')
            ...
            Log::warning('ExecutiveDashboard error: ' . $e->getMessage());
        }

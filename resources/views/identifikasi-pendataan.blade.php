                    ...
                    <a href="{{ request()->fullUrlWithQuery(['kategori' => 'under_80']) }}" 
                       class="card qc-stat-card shadow-sm bg-danger-lt {{ $filterKategori === 'under_80' ? 'active-filter' : '' }}">
                        <div class="card-body p-3">
                            ...
                            <div class="text-muted small d-flex flex-column gap-1 mt-1" style="font-size: 0.72rem;">
                                ...
                                @endif
                                @if($hideNonSls)
                                    <div><span class="badge bg-blue-lt text-blue py-0 px-1 font-weight-normal">Non-SLS Di-hide</span></div>
                                @endif
                            </div>
                        </div>
                    </a>
                    ...
                    <form method="GET" action="{{ route('identifikasi.pendataan') }}" class="row g-2 align-items-end">
                        ...
                        <div class="col-12 col-sm-6 col-lg-3">
                            ...
                            <select name="kategori" class="form-select form-select-sm font-weight-bold">
                                <option value="anomali_only" {{ (string) $filterKategori === 'anomali_only' ? 'selected' : '' }}>⚠️ Semua SLS Anomali / Perlu QC ({{ number_format($summary['cnt_total_anomali'] ?? 0) }})</option>
                                <option value="all" {{ (string) $filterKategori === 'all' ? 'selected' : '' }}>📋 Semua SLS ({{ number_format($summary['total_sls'] ?? 0) }})</option>
                                <option value="under_80" {{ (string) $filterKategori === 'under_80' ? 'selected' : '' }}>🚨 Muatan Murni &lt; 80% Prelist ({{ number_format($summary['cnt_under_80'] ?? 0) }})</option>
                                <option value="saved_wilkerstat" {{ (string) $filterKategori === 'saved_wilkerstat' ? 'selected' : '' }}>🛡️ Lolos Toleransi Wilkerstat &ge;90% ({{ number_format($summary['cnt_saved_by_wilkerstat'] ?? 0) }})</option>
                                <option value="over_130" {{ (string) $filterKategori === 'over_130' ? 'selected' : '' }}>📈 Lonjakan Muatan &gt; 130% Prelist ({{ number_format($summary['cnt_over_130'] ?? 0) }})</option>
                                <option value="zero_usaha" {{ (string) $filterKategori === 'zero_usaha' ? 'selected' : '' }}>🟣 Usaha SE Nol ({{ number_format($summary['cnt_zero_usaha'] ?? 0) }})</option>
                                <option value="usaha_drop" {{ (string) $filterKategori === 'usaha_drop' ? 'selected' : '' }}>📉 Usaha Drop vs Wilkerstat &gt;30% ({{ number_format($summary['cnt_usaha_drop'] ?? 0) }})</option>
                                <option value="keluarga_drop" {{ (string) $filterKategori === 'keluarga_drop' ? 'selected' : '' }}>🔴 Keluarga Tdk Ditemukan &ge;15% ({{ number_format($summary['cnt_keluarga_drop'] ?? 0) }})</option>
                                <option value="ganda" {{ (string) $filterKategori === 'ganda' ? 'selected' : '' }}>👥 Khusus Ganda Usaha ({{ number_format($summary['cnt_ganda'] ?? 0) }})</option>
                                <option value="bangunan_lainnya" {{ (string) $filterKategori === 'bangunan_lainnya' ? 'selected' : '' }}>🏚️ Bangunan Kosong/Lainnya &ge;15% ({{ number_format($summary['cnt_bangunan_lainnya'] ?? 0) }})</option>
                                <option value="aman" {{ (string) $filterKategori === 'aman' ? 'selected' : '' }}>✅ SLS Wajar / Aman ({{ number_format($summary['cnt_aman'] ?? 0) }})</option>
                            </select>
                        </div>
                        ...
                        <div class="col-6 col-sm-6 col-lg-2">
                            ...
                            <select name="tipe_wilayah" class="form-select form-select-sm font-weight-medium">
                                <option value="sls" {{ in_array((string) $filterTipeWilayah, ['sls', 'pemukiman']) ? 'selected' : '' }}>🏡 SLS Penduduk (Default)</option>
                                <option value="all" {{ (string) $filterTipeWilayah === 'all' ? 'selected' : '' }}>🌐 Semua Wilayah ({{ number_format($summary['total_all_sls'] ?? 0) }})</option>
                                <option value="non_sls" {{ in_array((string) $filterTipeWilayah, ['non_sls', 'non_pemukiman']) ? 'selected' : '' }}>🌾 Hanya Non-SLS ({{ number_format($summary['cnt_non_sls'] ?? 0) }})</option>
                            </select>
                        </div>
                        ...
                        <div class="col-6 col-sm-6 col-lg-2">
                            ...
                            <select name="kodekec" class="form-select form-select-sm font-weight-medium">
                                <option value="">Semua Kecamatan</option>
                                @foreach($kecNameMap as $kecCode => $kecName)
                                    <option value="{{ $kecCode }}" {{ (string) $kodekec === (string) $kecCode ? 'selected' : '' }}>
                                        {{ $kecName }} ({{ $kecCode }})
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-6 col-sm-6 col-lg-2">
                            ...
                            <select name="status_submit" class="form-select form-select-sm">
                                <option value="all" {{ (string) $statusSubmitFilter === 'all' ? 'selected' : '' }}>Semua Status</option>
                                <option value="completed" {{ (string) $statusSubmitFilter === 'completed' ? 'selected' : '' }}>Selesai (100%)</option>
                                <option value="in_progress" {{ (string) $statusSubmitFilter === 'in_progress' ? 'selected' : '' }}>Berjalan (&gt;0% &lt;100%)</option>
                                <option value="open" {{ (string) $statusSubmitFilter === 'open' ? 'selected' : '' }}>Belum Mulai (0%)</option>
                            </select>
                        </div>
                        ...
                        <div class="col-6 col-sm-6 col-lg-1">
                            ...
                            <select name="tanggal_data" class="form-select form-select-sm">
                                @foreach($availableDates as $d)
                                    <option value="{{ $d }}" {{ (string) $selectedDate === (string) $d ? 'selected' : '' }}>
                        ...
                        <div class="col-12 col-lg-auto d-flex gap-1 align-items-end ms-auto">
                            ...
                            <a href="{{ route('identifikasi.pendataan') }}" class="btn btn-outline-secondary btn-sm px-2.5 d-inline-flex align-items-center gap-1" title="Reset Filter ke Default">
            ...
            <div class="card border-0 shadow-sm" style="border-radius: 12px; overflow: hidden;">
                <div class="card-header bg-white py-3 d-flex flex-wrap justify-content-between align-items-center gap-2 border-bottom">
                    ...
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        ...
                        <span class="text-muted small ms-1">Prelist: <strong>{{ number_format($records->sum('jml_prelist')) }}</strong> | Murni: <strong>{{ number_format($records->sum('muatan_murni')) }}</strong> | Wilkerstat: <strong>{{ number_format($records->sum('wilkerstat_muatan')) }}</strong></span>
                    </div>
                </div>
                        ...
                        <table id="qc-table" class="table table-vcenter table-striped card-table table-qc text-nowrap w-100">
                            <thead>
                                <tr class="bg-light text-muted">
                                    ...
                                    <th class="text-center font-weight-bold">Rasio Murni<br><span class="font-weight-normal small">vs Prelist &amp; Wilkerstat</span></th>
...
$pWilkerstat = $pctMurniWilk !== null ? number_format($pctMurniWilk, 1) . '%' : '-';
$pSubmit = number_format($row->pct_submit ?? 0, 1) . '%';
$isSavedByWilkerstat = !empty($row->is_saved_by_wilkerstat);
$isUnder80 = !empty($row->is_under_80);
$hasAnomali = !empty($row->has_anomali);
$jmlPrelist = (int) ($row->jml_prelist ?? 0);
$muatanMurni = (int) ($row->muatan_murni ?? 0);
$wilkerstatMuatan = (int) ($row->wilkerstat_muatan ?? 0);
@endphp
<tr>
...
<td class="text-end font-weight-extrabold text-teal bg-teal-lt fs-3" data-order="{{ $muatanMurni }}">{{ number_format($muatanMurni) }}<div class="small text-muted font-weight-normal" style="font-size: 0.68rem;">@if($muatanMurni == 0)@if(!empty($row->is_non_sls))<span class="text-secondary opacity-75">Non-SLS (Sawah/Perairan)</span>@else<span class="text-danger font-weight-bold">0 Ditemukan</span>@endif @else KK: {{ number_format($row->pk_ditemukan ?? 0) }} | BKU: {{ number_format($row->up_ditemukan ?? 0) }}@endif</div></td>
<td class="text-center" data-order="{{ $row->rasio_order ?? 99999 }}">@if($jmlPrelist > 0)@if($isSavedByWilkerstat)<span class="badge bg-teal-lt text-teal font-weight-bold px-2 py-1 fs-4 shadow-xs" title="Murni <80% Prelist ({{ $pMurni }}), tapi sudah ≥90% Wilkerstat ({{ $pWilkerstat }}) sehingga AMAN">🛡️ {{ $pMurni }}</span><div class="small text-teal font-weight-bold mt-0.5" style="font-size: 0.68rem;">vs Wil: {{ $pWilkerstat }} (Aman)</div>@elseif($isUnder80)<span class="badge bg-danger text-white font-weight-extrabold px-2 py-1 fs-4 shadow-xs" title="Muatan Murni Kurang (< 80% Prelist & < 90% Wilkerstat)">🚨 {{ $pMurni }}</span>@if($pctMurniWilk !== null)<div class="small text-muted font-weight-normal mt-0.5" style="font-size: 0.68rem;">vs Wil: {{ $pWilkerstat }}</div>@endif @elseif(($pctMurniPrelist ?? 0) > 130.0)<span class="badge bg-warning text-dark font-weight-extrabold px-2 py-1 fs-4 shadow-xs" title="Muatan Murni Melonjak (> 130% Prelist)">📈 {{ $pMurni }}</span>@if($pctMurniWilk !== null)<div class="small text-muted font-weight-normal mt-0.5" style="font-size: 0.68rem;">vs Wil: {{ $pWilkerstat }}</div>@endif @else<span class="badge bg-success-lt text-success font-weight-bold px-2 py-1 fs-4" title="Rasio Normal">✅ {{ $pMurni }}</span>@if($pctMurniWilk !== null)<div class="small text-muted font-weight-normal mt-0.5" style="font-size: 0.68rem;">vs Wil: {{ $pWilkerstat }}</div>@endif @endif @else @if($muatanMurni > 0)<span class="badge bg-info-lt text-info font-weight-bold px-2 py-1" title="SLS Pemekaran Baru">Baru ({{ number_format($muatanMurni) }})</span>@if($pctMurniWilk !== null)<div class="small text-muted font-weight-normal mt-0.5" style="font-size: 0.68rem;">vs Wil: {{ $pWilkerstat }}</div>@endif @else<span class="badge bg-light text-muted border px-2 py-1" title="Prelist Awal 0 & Muatan 0">Nol Prelist</span>@endif @endif</td>

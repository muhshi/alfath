...
- **Executive Public Dashboard Sensus Ekonomi 2026**:
  ...
  - Perbaikan Penanganan Docker (`deploy.sh`): Memprioritaskan **Docker Compose V2 (`docker compose`)** dibandingkan versi lama Python v1 (`docker-compose 1.29`), serta menambahkan penghapusan container macet otomatis (`docker rm -f`) untuk mencegah error `KeyError: ContainerConfig` dari skrip Python Docker v1 lama saat membatalkan build.

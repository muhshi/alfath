...
- **Optimasi Performa Database & Pemulihan Tab Ranking Kinerja Petugas**:
  ...
  - **Koreksi Perhitungan Muatan Murni & Agregasi Subquery Multi-Tanggal (`tanggal_data`)**:
    - Memperbaiki bug pada `Se2026MonitoringService` dan `PetugasPerformanceRankingService` yang menyebabkan nilai *Muatan Murni* (dan metrik Usaha) membengkak 2x lipat/terhitung ganda akibat subquery `se2026_usaha_perusahaan`, `se2026_usaha_keluarga`, dan `se2026_pemutakhiran_keluarga` yang melakukan `SUM(...)` tanpa memfilter `tanggal_data` snapshot.

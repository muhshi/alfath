...
- **Peningkatan Dashboard Pengolahan SE2026 (`/dashboard-pengolahan`)**:
  - **Kalkulasi Stagnan Dinamis & Presisi Stagnan 1 Hari**: Menghitung secara fleksibel jumlah hari/snapshot berturut-turut di mana submit petugas tidak bertambah (`🚨 Stagnan X hari`, misal: Stagnan 1 hari, Stagnan 5 hari, Stagnan 10 hari). Petugas dengan `submit 0/hari` pada snapshot harian otomatis dikategorikan sebagai `🚨 Stagnan N hari` (bukan Progres Lambat), sedangkan status `⚠️ Progres Lambat` khusus untuk petugas yang ada penambahan submit (`submit > 0`) tetapi di bawah target harian (< 5/hari).

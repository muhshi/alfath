...
- **Peningkatan Dashboard Pengolahan SE2026 (`/dashboard-pengolahan`)**:
  - **Kalkulasi Stagnan Dinamis & Presisi Progres Lambat**: Menghitung secara fleksibel jumlah hari/snapshot berturut-turut di mana submit petugas tidak bertambah (`🚨 Stagnan X hari`, misal: Stagnan 1 hari, Stagnan 5 hari, Stagnan 10 hari). Petugas dengan `submit 0/hari` pada snapshot harian otomatis dikategorikan sebagai `🚨 Stagnan N hari` (bukan Progres Lambat). Status `⚠️ Progres Lambat` kini khusus diberikan jika petugas ada penambahan submit hari ini (`submit > 0`), namun nilainya kurang dari target submit di kolom **Laju s.d. 20 Agustus** (`submit_today < laju_harian_95`).
  - **Tampilan Submit Harian (`submit X/hari`) pada Cell Status Warning**: Menampilkan informasi laju submit selisih harian (`total_submit_hari_ini - total_submit_kemarin`) di bagian atas cell kolom Status Warning di tabel Ranking Kinerja Petugas (`submit X/hari`), di atas badge status (`Stagnan X hari`, `Progres Lambat`, `On-Track`, atau `Selesai 100%`).

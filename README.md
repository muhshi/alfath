...
### 2026-08-07
- **Tabel Petugas SE2026 (DataTables Integration, Nav Tabs & Multi-Table SLS Name Resolution)**:
  - Resolusi Nama SLS Multi-Tabel (`COALESCE` Fallback): Memperbarui kueri `getSlsQuery` untuk mengambil nama SLS secara berlapis dari 3 tabel (`monitoring_sls_se2026.nmsls`, `sipw.nama_sls`, dan `se2026_pemutakhiran_keluarga.sub_sls`). Jika nama SLS bernilai `NULL` atau `-`, sistem otomatis mengambil nama dari Wilkerstat / Pemutakhiran Keluarga atau menampilkan format `SLS [Kode]` sehingga tidak ada lagi nama SLS yang tampil sebagai `-`.
